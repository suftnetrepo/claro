...
import {
  ...
  StyledDivider, StyledDatePicker, Popup, TabBar,
} from 'fluent-styles'
  ...
  return (
    <Stack flex={1} backgroundColor={Colors.bg}>
      ...
      <Stack flex={1} paddingHorizontal={20} paddingTop={8}>
        ...
        {/* Account */}
        <StyledPressable flexDirection="row" alignItems="center" gap={14} paddingVertical={14} onPress={() => setShowAccount(true)}>
        ...
        {txType === 'transfer' && (
          <>
            <StyledPressable flexDirection="row" alignItems="center" gap={14} paddingVertical={14} onPress={() => setShowToAccount(true)}>
              <StyledText fontSize={18} width={28} textAlign="center">🔄</StyledText>
        ...
        {txType !== 'transfer' && (
          <>
            <StyledPressable flexDirection="row" alignItems="center" gap={14} paddingVertical={14} onPress={() => setShowCategory(true)}>
        ...
        {/* Date */}
        <StyledPressable flexDirection="row" alignItems="center" gap={14} paddingVertical={14} onPress={() => setShowDate(true)}>
        ...
        <Stack flexDirection="row" alignItems="flex-start" gap={14} paddingVertical={8}>
          ...
          <StyledTextInput
            flex={1} placeholder="Add notes..." value={notes} onChangeText={setNotes}
            variant="ghost" multiline numberOfLines={2}
            placeholderTextColor={Colors.textMuted} fontSize={15} color={Colors.textPrimary}
          />
        </Stack>
      ...
      {/* Date popup */}
      <Popup visible={showDate} onClose={() => setShowDate(false)} title="Select date" showClose position="bottom" round>
        <Stack paddingHorizontal={16} paddingBottom={24}>
          <StyledDatePicker
            mode="date" variant="inline" value={date} onChange={setDate} showTodayButton
            colors={{ selected: accentColor, today: accentColor, confirmBg: accentColor }}
          />
          <StyledPressable paddingVertical={14} borderRadius={12} backgroundColor={accentColor} alignItems="center" marginTop={8} onPress={() => setShowDate(false)}>
      ...
      <CategoryPicker visible={showCategory} expenseCategories={expenseCategories} incomeCategories={incomeCategories} selected={categoryId} transactionType={txType === 'transfer' ? 'expense' : txType} onSelect={c => setCategoryId(c.id)} onClose={() => setShowCategory(false)} />
